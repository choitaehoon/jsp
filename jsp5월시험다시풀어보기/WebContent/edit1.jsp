<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import=" java.net.*, java.util.*, p201432038.*"%>
<%
	request.setCharacterEncoding("UTF-8");
	String 에러메시지 = null;
	String s1 = request.getParameter("id"); //가져온 아이디
	int id = ParseUtils.parseInt(s1, 0);
	String pg = request.getParameter("pg"); //page번호
	...
	if (request.getMethod().equals("GET")) {
		...
		article.setNotice(request.getParameter("notice") != null);
...
<body>
	<div class="container">
		...
		<form method="post">
			...
			<div class="form-group">
				<label>공지</label> <input type="checkbox" class="form-control"
					name="notice" <%= article.isNotice() ? "checked" : "" %> />
			</div>
			<button type="submit" class="btn btn-primary">
				<i class="glyphicon glyphicon-ok"></i> 저장
			</button>
			<a
				href="studentDelete1.jsp?id=<%=id%>&pg=<%=pg%>&ss=<%=ss%>&st=<%=stEncoded%>&od=<%=od%>"
				class="btn btn-danger" onclick="return confirm('삭제하시겠습니까?')"> <i
				class="glyphicon glyphicon-trash"></i> 삭제
			</a> <a
				href="studentList1.jsp?pg=<%=pg%>&ss=<%=ss%>&st=<%=stEncoded%>&od=<%=od%>"
