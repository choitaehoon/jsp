...
<%
	...
	String sofe = "소프".equals(ss) ? "selected" : "";
	String computer = "컴퓨".equals(ss) ? "selected" : "";
	String inter = "정보".equals(ss) ? "selected" : "";
	String gt = "글로".equals(ss) ? "selected" : "";
	...
	if (request.getMethod().equals("POST")) {
		...
		if (userid == null || userid.length() == 0)
			...
			User user = new User(userid, name, password1, email, ss);
		...
		<form method="post">
			...
			<div class="form-group">
				<label>학과</label> <select class="form-control" name="department">
					<option value="소프" <%=sofe%>>소프트웨어공학과</option>
					<option value="컴퓨" <%=computer%>>컴퓨터공학과</option>
					<option value="정보" <%=inter%>>정보통신공학과</option>
					<option value="글로" <%=gt%>>글로컬IT공학과</option>
				</select>
			</div>
